...
- **Feature Deletion**: Completely removed Export, Import, and Comment features from the application per user request. This involved dropping the `Comments` database table via EF Core migration, deleting Domain entities, Application commands/handlers, API controllers, frontend services, and UI components from `todo-detail.component` and `todo-list.component`.

### 🧠 Key Decisions & Why
- **Clean Feature Removal**: Ensured no dead code remained by completely tracing the feature stack from the UI signals and components down to the MediatR handlers, Domain Entities, and finally applying an EF Core migration to safely drop the underlying tables without breaking the rest of the database schema.
